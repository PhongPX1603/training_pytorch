...
class RegionDataset(Dataset):
    def __init__(self, dirname, classes, image_size, image_pattern, mask_pattern, is_textline=False, ignore_field = [],
        ...
        images = natsorted(dirname.glob('{}'.format(image_pattern)), key=lambda x: str(x))
        masks = natsorted(dirname.glob('{}'.format(mask_pattern)), key=lambda x: str(x))
        self.data_pairs = [(image, mask)for image, mask in zip(images, masks)]
        
        print('--' * 20)
        print(f'[___Dataset Info___]: {dirname.stem} set - {len(self.data_pairs)} pairs.')
