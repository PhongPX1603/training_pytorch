...
class SqueezeExcitation(nn.Module):
    ...
    def _scale(self, input: Tensor, inplace: bool) -> Tensor:
        scale = F.adaptive_avg_pool2d(input, 1)  # B, C, 1, 1
        scale = self.fc1(scale)  # B, C / squeeze_factor, 1, 1
        scale = self.relu(scale)  # B, C / squeeze_factor, 1, 1
        scale = self.fc2(scale)  # B, C, 1, 1
        return F.hardsigmoid(scale, inplace=inplace)  # B, C, 1, 1
